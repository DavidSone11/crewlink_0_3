(function() {
	...
	var DEBUG = function(val) {
		if (isDebug) {
			("SpringDataRestApi : LOG : ");
			(val);
		}
	};
	var ERROR = function(val) {
		("SpringDataRestApi : ERROR : ");
		(val);
	};
											...
											angular.forEach(resultItems,function(item) {
												angular.forEach(subLinks, function(subLink) {
													if (item._links[subLink]) {
														...
														item._ref[subLink] = {};
														promises.push(service.get(item._links[subLink].href,newSubLinks,true).then(function(response){
															item._ref[subLink] = response;
														}));
													} else {
														ERROR("SubLink : "+ subLink+ " not found in "+ item._links.self.href);
...
//											deferred.resolve(result);
											...
											angular.forEach(subLinks,function(subLink) {
												if (result._links[subLink]) {
													...
													result._ref[subLink] = {};
													promises.push(service.get(result._links[subLink].href,newSubLinks,true).then(function(response){
...
//									}
					...
					service.deleteItem = function(link){
						var deferred = $q.defer();
						if(!link){
							deferred.reject('DELETE Failed Error on Href : '+ link+ ' : '+ response.status+ ' : '+ response.statusText);
						}else{
							$http.delete(link).then(
								function(response){
									deferred.resolve(response);
								}.bind(this),
								function(response){
									ERROR('DELETE Error on Href : '+ link+ ' : '+ response.status+ ' : '+ response.statusText);
									DEBUG(response);
									deferred.reject('DELETE Error on Href : '+ link + ' : '+ response.status+ ' : '+ response.statusText);
								}.bind(this)
							);
						}
						return deferred.promise;
...
//this.baseUrl = RestApi.users;

//this.getUserById = function(id,expandFields){
...
//}
//SpringDataRestAdapter.process($http.get(this.baseUrl),expandFields,isRecursive).then(function(processedResponse){
//result = processedResponse._embeddedItems;
//if(expandFields.subFields){
//angular.forEach(expandFields.subFields, function(subField) {
//SpringDataRestAdapter.process($http.get(user[expanedFields.subFields])
//}
//},this);
//return result;
//};

//SpringDataRestAdapter.process(httpPromise,['headStation','role'],
//true).then(function (processedResponse) {
//$scope.users = processedResponse._embeddedItems;

//angular.forEach($scope.users, function(user) {
//var httpPromise =
//$http.get(user.headStation._links.station.href).success(function (response) {
//user.headStation.station = response;//angular.toJson(response, true);
//(user);
//});
//},this);
